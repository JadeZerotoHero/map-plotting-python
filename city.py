...
class City():
    ...
    def __init__(self, name: str, coordinates: Tuple[float, float], city_type: str,\
        ...
        if name not in City.name_to_cities:
            City.name_to_cities[name] = []
    ...
    def distance(self, other_city: City) -> int:
        ...
        """
        #TODO
        cityc1 = self.coordinates
        cityc2 = other_city.coordinates
        dist_km = geopy.distance.great_circle(cityc1, cityc2).km
        return math.ceil(dist_km)
    ...
    def get_table_data(self) -> list[str]:
        ...
        """
        #TODO
        coordinates_str = f"({self.coordinates[0]}, {self.coordinates[1]})"
        population_str = str(self.population)
        city_id_str = str(self.city_id)
        return [self.name, coordinates_str, self.city_type, population_str, city_id_str]
